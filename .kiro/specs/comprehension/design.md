...
| Decidido (2026-07-09) — **text-first; voz DIFERIDA** | Transcripción de voz: ¿cliente o F2? | Al encender el worker, F2 procesa capturas de **texto** end-to-end. La voz queda **diferida**: el pipeline ya deja un *seam* seguro (`_resolve_text`) que, si una captura no trae `body` y no hay `audio_loader`, lanza un error CLARO y la captura se preserva intacta y queda `failed`/reprocesable (P-COMP-5) — nunca se pierde. Implementar la voz es un sub-proyecto propio (requiere `AIProvider.transcribe` real + acceso al blob de audio por presigned GET) y se decidirá **cliente vs F2** con datos de calidad/latencia. No bloquea el encendido del motor para texto. |
